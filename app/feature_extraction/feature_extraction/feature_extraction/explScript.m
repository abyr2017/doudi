...
[input_mat,target_mat] = prprob;

%%
% Here A, the first letter, is plotted as a bit map.

plotchar(input_mat(:,1))
...
net1.divideFcn = '';
net1 = train(net1,input_mat,target_mat,nnMATLAB);
...
numNoise = 30;
Xn = min(max(repmat(input_mat,1,numNoise)+randn(35,26*numNoise)*0.2,0),1);
Tn = repmat(target_mat,1,numNoise);
...
for i = 1:numLevels
  Xtest = min(max(repmat(input_mat,1,numNoise)+randn(35,26*numNoise)*noiseLevels(i),0),1);
  Y1 = net1(Xtest);
