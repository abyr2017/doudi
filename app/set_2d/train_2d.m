...
%net1.divideFcn = '';net.divideFcn = 'dividerand';  % Divide data randomly
net_2d.divideMode = 'sample';  % Divide up every sample
net_2d.divideParam.trainRatio = 70/100;
net_2d.divideParam.valRatio = 15/100;
net_2d.divideParam.testRatio = 15/100;

net_2d.plotFcns = {'plotperform','plottrainstate','ploterrhist', ...
    'plotconfusion', 'plotregression'};



net_2d = train(net_2d,X,T,nnMATLAB);
y = net_2d(X);
e = gsubtract(T,y);
performance = perform(net_2d,T,y)
tind = vec2ind(T);
yind = vec2ind(y);
